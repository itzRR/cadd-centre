...
export default function AdminDashboard() {
      ...
      return (
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <button onClick={loadStats} className="mt-4 px-4 py-2 bg-red-600 text-white rounded-xl text-sm font-medium hover:bg-red-700 transition-colors">
              Retry
            </button>
          </div>
        </div>
  ...
  const statCards = [
    { title: "Total Students",    value: stats.totalStudents,      icon: Users,         gradient: "from-red-500 to-red-600",      bg: "bg-red-50",  text: "text-red-700",  href: "/admin/students" },
  ...
  const adminGuideSteps: GuideStep[] = [
    { title: "Welcome to ASMS Dashboard", description: "This is your central hub for managing the entire academic system. View real-time stats, revenue, enrollments, and student lifecycle - all at a glance.", icon: GraduationCap, gradient: "from-red-500 to-cyan-500", tip: "Stats auto-refresh every 30 seconds and update in real-time when changes happen." },
    { title: "Managing People", description: "Under the 'People' group in the sidebar, you'll find Leads (prospective students), Students (enrolled), and Lecturers. Click any stat card to jump directly to that section.", icon: Users, gradient: "from-red-500 to-red-600", tip: "Use the sidebar groups - click a category to expand it and see its items." },
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-red-600 to-cyan-500 flex items-center justify-center shadow-lg shadow-red-500/20">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <QuickGuide
            ...
            accentGradient="from-red-600 to-cyan-500"
          ...
          <Link href="/admin/batches" className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-600 to-cyan-500 hover:from-red-500 hover:to-cyan-400 text-white rounded-xl text-sm font-bold transition-all shadow-md shadow-red-500/20 hover:shadow-lg">
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {statCards.map((stat, i) => (
          <motion.div variants={item} key={i}>
            <Link href={stat.href}>
              <div className="group bg-white rounded-2xl p-4 md:p-5 border border-gray-100 hover:border-red-200 hover:shadow-lg hover:shadow-red-500/5 transition-all duration-300 cursor-pointer relative overflow-hidden">
                {/* Background accent */}
              ...
              >
                ...
                <div className="flex-1 bg-gray-100 rounded-full h-3 overflow-hidden">
                  <motion.div
                    ...
                    className="h-full rounded-full bg-gradient-to-r from-red-500 to-cyan-400 group-hover:from-red-400 group-hover:to-cyan-300 transition-colors"
                  />
                </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="px-6 pt-5 pb-3 border-b border-gray-50">
            <div className="flex items-center justify-between">
              ...
              <Link href="/admin/enrollments" className="text-xs font-bold text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-colors">
                View All →
              </Link>
            </div>
          </div>
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                {stats.recentEnrollments.slice(0, 6).map((enrollment: any, i: number) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.5 + i * 0.08 }}
                    className="flex items-center justify-between p-3 rounded-xl bg-gray-50 hover:bg-red-50/50 border border-transparent hover:border-red-100 transition-all group"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-9 h-9 rounded-full bg-gradient-to-br from-red-500 to-cyan-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                    ...
                    <div className="text-right flex-shrink-0 ml-3">
                      ...
                      <Badge className={`text-[10px] mt-0.5 font-bold ${
                        ...
                        enrollment.status === 'confirmed' ? 'bg-red-100 text-red-700 border-red-200' :
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
        ...
        <div className="flex items-center gap-2 mb-4">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-red-600 to-cyan-500 flex items-center justify-center">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {[
            { step: "1", label: "Registration",       href: "/admin/students",     color: "from-red-500 to-red-600" },
            ...
            <motion.div key={s.step} className="flex items-center gap-2"
              ...
              <Link href={s.href}
                className="flex items-center gap-2 bg-gray-50 hover:bg-red-50 border border-gray-100 hover:border-red-200 text-gray-700 hover:text-red-700 rounded-xl px-3 py-2 text-xs md:text-sm font-semibold transition-all hover:shadow-sm">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}
        ...
        <div className="px-6 pt-5 pb-3 border-b border-gray-50 flex items-center justify-between">
          ...
          <Link href="/admin/enrollments" className="text-xs font-bold text-red-600 hover:text-red-700 transition-colors">
            View all →
          </Link>
        </div>
              ...
              {stats.recentEnrollments.length === 0 ? (
                ...
                <tr key={e.id} className="hover:bg-red-50/30 transition-colors">
                  <td className="py-3.5 px-6">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-red-500 to-cyan-500 flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0">
                  ...
                  <td className="py-3.5 px-6">
                    <Badge className={`text-[10px] font-bold ${
                      ...
                      e.status === "confirmed" ? "bg-red-100 text-red-700 border-red-200" :
                      "bg-yellow-100 text-yellow-700 border-yellow-200"
                    }`}>{e.status}</Badge>
                  </td>
