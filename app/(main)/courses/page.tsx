...
const levelColor = (l: string) => l === "Expert Certificate" ? "bg-purple-100 text-purple-800" : l === "Master Certificate" ? "bg-red-100 text-red-800" : "bg-emerald-100 text-emerald-800"
...
export default function CoursesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] selection:bg-red-500/30">
      {/* ── PREMIUM HERO ────────────────────────────────────────────── */}
      <section className="relative pt-32 pb-24 bg-[#F8FAFC] overflow-hidden border-b border-gray-100">
        <div className="absolute top-[10%] left-[10%] w-[600px] h-[600px] bg-red-300/30 blur-[120px] rounded-full pointer-events-none mix-blend-multiply animate-pulse" style={{ animationDuration: '8s' }} />
        ...
        <motion.div animate={{ y: [0, -30, 0], rotate: [0, 45, 0] }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} className="absolute top-20 right-20 w-32 h-32 bg-white/40 backdrop-blur-3xl rounded-[2rem] border border-white/60 shadow-xl hidden lg:flex items-center justify-center">
           <Hexagon className="w-12 h-12 text-red-500/40" />
        </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="text-center max-w-4xl mx-auto">
            <span className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-md border border-red-100 text-red-700 text-sm font-bold tracking-widest uppercase px-5 py-2 rounded-full mb-8 shadow-sm">
            ...
            <h1 className="text-5xl md:text-7xl lg:text-[5rem] font-black text-[#0F172A] mb-6 tracking-tight leading-[1.05]">
              Find Your Next <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-cyan-500">Mastery Level</span>
            </h1>
            ...
            <div className="relative max-w-2xl mx-auto group">
              <div className="absolute -inset-2 bg-gradient-to-r from-red-400 to-cyan-300 rounded-[2rem] blur-lg opacity-20 group-focus-within:opacity-50 transition-opacity duration-500" />
              <div className="relative flex items-center bg-white/90 backdrop-blur-xl border border-white rounded-2xl p-2.5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] focus-within:ring-4 focus-within:ring-red-500/20 transition-all duration-300">
                <div className="w-12 h-12 bg-red-50 rounded-xl flex items-center justify-center mr-2 flex-shrink-0 text-red-600 group-focus-within:bg-red-600 group-focus-within:text-white transition-colors duration-300">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="lg:hidden w-full">
             <Button onClick={() => setShowFilters(!showFilters)} className="w-full h-14 rounded-2xl bg-white border border-gray-200 text-[#0F172A] shadow-sm flex justify-between items-center px-6">
                <span className="flex items-center gap-2 font-bold"><SlidersHorizontal className="w-5 h-5 text-red-600"/> Filters</span>
              ...
              >
                <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] p-6 border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                  <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
                    <h3 className="font-black text-xl text-[#0F172A] flex items-center gap-2"><SlidersHorizontal className="w-5 h-5 text-red-600" /> Filters</h3>
                    {(level !== "All" || category !== "All") && (
                      <button onClick={() => { setLevel("All"); setCategory("All") }} className="text-sm font-bold text-red-600 hover:text-red-700 bg-red-50 px-3 py-1.5 rounded-full transition-colors">Clear</button>
                    )}
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-2">
                        {CATEGORIES.map(c => {
                           ...
                           return (
                             <button key={c.name} onClick={() => setCategory(c.name)} className={`group relative text-left px-4 py-3 rounded-xl text-sm font-bold transition-all duration-300 flex items-center gap-3 overflow-hidden ${isSelected ? "bg-red-600 text-white shadow-md shadow-red-600/20" : "text-[#475569] hover:bg-white hover:shadow-sm border border-transparent hover:border-gray-100"}`}>
                               <c.icon className={`w-4 h-4 transition-transform duration-300 group-hover:scale-110 ${isSelected ? "text-red-200" : "text-gray-400 group-hover:text-red-600"}`} />
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-2">
                        {LEVELS.map(l => {
                           ...
                           return (
                             <button key={l} onClick={() => setLevel(l)} className={`group text-left px-4 py-3 rounded-xl text-sm font-bold transition-all duration-300 flex items-center justify-between ${isSelected ? "bg-red-50 text-red-700 border border-red-100" : "text-[#475569] hover:bg-white hover:shadow-sm border border-transparent hover:border-gray-100"}`}>
                               {l}
                               {isSelected && <motion.div layoutId="level-indicator" className="w-2.5 h-2.5 rounded-full bg-red-600 shadow-sm" />}
                             </button>
                           )
                        })}
                      </div>
          ...
          <div className="w-full lg:w-3/4">
            ...
            {isLoading ? (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white rounded-[3rem] border border-gray-100 p-20 text-center shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
                ...
                <Button onClick={() => { setSearch(""); setLevel("All"); setCategory("All") }} className="bg-red-600 hover:bg-red-700 text-white rounded-full h-14 px-10 font-bold shadow-lg hover:shadow-red-600/30 transition-all duration-300">
                  Reset Search
                </Button>
              </motion.div>
            ) : (
                      ...
                      <Link href={`/courses/${course.slug}`} className="group block bg-white rounded-[2rem] border border-gray-100 p-3 hover:shadow-[0_20px_50px_-10px_rgba(0,0,0,0.1)] transition-all duration-500 hover:-translate-y-2 h-full flex flex-col relative overflow-hidden">
                        <div className="h-64 rounded-[1.5rem] overflow-hidden relative mb-5 flex-shrink-0">
                          ...
                          <div className="absolute inset-0 z-30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <span className="bg-white text-red-600 px-6 py-3 rounded-full font-bold shadow-[0_10px_30px_rgba(0,0,0,0.2)] translate-y-4 group-hover:translate-y-0 transition-all duration-300">View Details</span>
                          </div>
                        </div>
                        ...
                        <div className="px-5 pb-5 flex flex-col flex-1">
                          <div className="flex items-center justify-between mb-3">
                             <p className="text-xs font-black text-red-600 uppercase tracking-widest flex items-center gap-1.5">
                               <span className="w-1.5 h-1.5 rounded-full bg-red-600" /> {course.category}
                             </p>
                          ...
                          <h3 className="font-black text-2xl text-[#0F172A] mb-3 group-hover:text-red-600 transition-colors line-clamp-2 leading-tight">{course.title}</h3>
                          ...
                          <div className="flex items-center justify-between mt-auto">
                            ...
                            <div className="w-12 h-12 rounded-2xl bg-[#F8FAFC] border border-gray-100 text-gray-400 flex items-center justify-center group-hover:bg-red-600 group-hover:border-red-600 group-hover:text-white transition-all duration-300 group-hover:shadow-[0_8px_20px_rgb(37,99,235,0.3)]">
