...
CREATE TYPE public.permission_key AS ENUM (
  ...
  'ims_control_panel', 'ims_it', 'asms_full', 'task_delete'
);
...
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.profiles
    WHERE id = auth.uid()
    AND role::text IN ('super_admin','admin','academic_head','academic_officer',
                 'finance_head','finance_officer','marketing_head','marketing_officer',
                 'hr_head','hr_officer','it_head','it_officer','staff','lecturer')
    AND disabled = FALSE
  );
$$ LANGUAGE sql SECURITY DEFINER STABLE;
...
-- ============================================================================
-- ADD IT DEPARTMENT ROLES TO STAFF FUNCTIONS
-- ============================================================================

-- âš ï¸  IMPORTANT: YOU MUST RUN THIS IN TWO SEPARATE STEPS! âš ï¸ 

-- ==========================================
-- STEP 1: Highlight and run ONLY these two lines first:
-- ==========================================
ALTER TYPE public.user_role ADD VALUE IF NOT EXISTS 'it_head';
ALTER TYPE public.user_role ADD VALUE IF NOT EXISTS 'it_officer';


-- ==========================================
-- STEP 2: Highlight and run everything below this line SECOND:
-- ==========================================
CREATE OR REPLACE FUNCTION public.is_staff_member()
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
